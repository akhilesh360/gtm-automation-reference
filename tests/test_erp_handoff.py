...
def _quote(**kw):
    q = dict(quote_id="Q-T1", account_id="ACC-1", account_name="Globex Enterprise", product_id="PRD-ENT-PLATFORM", quantity=1,
...
def test_mock_erp_is_idempotent_by_quote(tmp_path):
    ...
    assert a["accepted_total"] == 450000.0 and erp.get_sales_order(a["sales_order_id"])["customer"] == "Globex Enterprise"
